...
<script type="text/javascript">
	...
	<table border="1" style=" border-collapse:collapse; width:100%;">
		<tr>
			<th width="80px"><p style="font-size:14px">全选/反选</p><input type="checkbox" id="all"/></th>
			<th>栏目名称</th>
			<th>栏目编号</th>
			<th>是否启用</th>
			<th>修改</th>
		...
		<c:if test="${status.index %2 ==0 }">style="background:white"</c:if>>
			<td><input type="checkbox" name="id" value="${catalog.id }" name="id" class="id"/></td>
			<td>${catalog.name }</td>
			<td>${catalog.code }</td>
			<td>
				<c:if test="${catalog.isInuse=='1' }"><p style="color:blue">启用</p></c:if>
			    <c:if test="${catalog.isInuse=='0' }"><p style="color:red">禁用</p></c:if>
			   <%-- <a href="javascript:void(0)" val="${catalog.id }" class="upd">修改</a>
			   <a href="javascript:void(0)" val="${catalog.id }" class="del">删除</a> --%>
			</td>
			<td><a href="javascript:void(0)" val="${catalog.id }" class="upd">修改</a></td>
		</tr>
		</c:forEach>
	</table>
